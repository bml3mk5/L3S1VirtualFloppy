...
enum FDC_8INCH_UNIT_MASKS {
    FDC_UNIT_MOTOR      = 0x08, ///< motor on/off
    FDC_UNIT_SIDE1      = 0x10, ///< side select
    FDC_UNIT_DDEN       = 0x20, ///< double density
    FDC_UNIT_FAULT_RES  = 0x40, ///< fault reset
    FDC_UNIT_FDC_MASK   = 0x80, ///< fdc mask
};

enum FDC_8INCH_SOME_MASKS {
    FDC_UNIT_DRQ        = 0x80,
    FDC_ACCESS_NG       = 0x01,
    FDC_NMI_MASK        = 0x01,
};
...
//      uint8_t now_reset : 1;
    ...
    uint8_t nmi_enable;
    uint8_t fdc_mask;   // 0x80: fdc mask  0x01: 
...
static FDC_8INCH g_fdc_8inch;
static FDC_8INCH *dev = &g_fdc_8inch;

// use bit7
static uint8_t MY_CORE1_GROUP(fdc_mask_core1);
...
void fdc_8inch_init()
{
    memset(&g_fdc_8inch, 0, sizeof(g_fdc_8inch));
...
void fdc_8inch_reset()
{
    ...
    dev->now_drq = FDC_UNIT_DRQ;
    fdc_set_drq(0);   // clear DRQ
    fdc_set_irq(0);
  
    fdc_8inch_unitsel_reset();
}
...
void fdc_8inch_unitsel_reset()
{
    ...
    dev->sid_num = 0;
    dev->density = 0;
    dev->fdc_mask = FDC_ACCESS_NG;
    dev->wait_ack = 0;
    disk_drive_motor_off();
//    dev->motor = 0;
    fdc_mask_core1 = 0;
    halt_signal_off(HALT_SIGNAL_8INCH);
}
...
static void __no_inline_not_in_flash_func(fdc_cancel_event)(int event)
{
    ...
    OUT_DEBUG_EV(_T("FDC8: Cancel EVENT:%d\n"), event);
}
...
static void __no_inline_not_in_flash_func(fdc_register_event)(int event, uint32_t usec)
{
    ...
    OUT_DEBUG_EV(_T("FDC8: Regist EVENT:%d id:%08x usec:%d\n"), event, dev->event_info[event].id, usec);
}
...
//  printf("FDC8: Regist EVENT_SEARCH_DATA\n");
    dev->now_search = 1;
}
...
//    uint32_t tim = to_us_since_boot(get_absolute_time()) & 0xffffffff;
//  printf("Search data: TIM:%u C:%d H:%d R:%d IDX:%d DAT:%02x STS:%02x DST:%02x\n"
//    , tim
//    , dev->trkreg, dev->sid_num, dev->secreg
//    , dev->data_idx, dev->datareg, dev->status, dev->drv_sts);
...
{
    // set prepared data to DR 
    msg_send_data_to_core1(MSG_TYPE_PARALLEL_NOTICE, MSG_NOTICE_FDC_COMMON, 0);
...
{
    dev->fdc_mask = ((data & FDC_UNIT_FDC_MASK) | (dev->fdc_mask & ~FDC_UNIT_FDC_MASK));
    fdc_common_set_disk_type_shift((data & FDC_UNIT_FDC_MASK) != 0);
//    if (dev->now_drq) {
//        msg_send_data_to_core1(MSG_TYPE_PARALLEL_WRITE, FDC_8INCH_DRQ, ~dev->now_drq);
//    }
...
{
//    printf("FDC8: w a:%02x d:%02x nm:%02x\n", addr, data, dev->nmi_enable);
    switch(addr & 15) {
    case FDC_MB8877_CR:
        // command reg
        if (!dev->fdc_mask) {
    ...
    case FDC_8INCH_HALT:
        // halt
        if (dev->now_drq | dev->now_irq) {
            halt_signal_off(HALT_SIGNAL_8INCH);
        } else {
            halt_signal_on(HALT_SIGNAL_8INCH);
//            uint32_t tim = to_us_since_boot(get_absolute_time()) & 0xffffffff;
//            printf("HALT ON: TIM:%u C:%d H:%d R:%d POS:%d STS:%02x DST:%02x\n"
//            , tim
//            , dev->trkreg, dev->sid_num, dev->secreg, dev->data_idx, dev->status, dev->drv_sts);
        }
        break;
    default:
        break;
...
//          fdc_send_status();
        ...
        dev->nmi_enable |= (~dev->fdc_mask & FDC_ACCESS_NG);
        break;
    case FDC_MB8877_TR:
        dev->nmi_enable |= (~dev->fdc_mask & FDC_ACCESS_NG);
        break;
    case FDC_MB8877_SCR:
        ...
        break;
    case FDC_MB8877_DR:
        // fdc_mask = 0x80 & now_drq = 0x80
        // fdc_nonmask = 0x00 & now_drq = 0x80
        if ((dev->fdc_mask & ~dev->now_drq) == 0) {
            switch(dev->cmdtype) {
            case FDC_CMD_RD_SEC:
            case FDC_CMD_RD_MSEC:
...
static bool MY_CORE1_FUNC(fdc_post_read_tightly)(uint8_t addr)
{
    ...
    switch(addr & 15) {
    case FDC_MB8877_DR:
        if ((fdc_mask_core1 & FDC_UNIT_FDC_MASK) == 0) {
            sts = true;
            // clear DRQ flag
            g_pio_parallel_read.odata[FDC_8INCH_DRQ] |= FDC_UNIT_DRQ;
        }
        break;
    case FDC_MB8877_STR:
    case FDC_MB8877_TR:
    case FDC_MB8877_SCR:
        ...
        break;
    case FDC_8INCH_DRQ:
        // set bit7 if DRQ is off, otherwise clear it.
        // bit6 - 0 doesn't have meaning.
        fdc_mask_core1 = g_pio_parallel_read.odata[FDC_8INCH_DRQ];
        break;
    default:
        break;
    }
...
static void MY_CORE1_FUNC(fdc_post_write_tightly)(uint8_t addr)
{
    ...
    case FDC_MB8877_CR:
        // command
//        if (pio_ctrls_get_disk_type_shift()) {
//            fdc_common_clr_irq_core1();
//        }
    ...
    case FDC_MB8877_DR:
        ...
        break;
    case FDC_8INCH_UNIT:
        pio_ctrls_set_disk_type_shift(g_pio_parallel_write.idata & FDC_UNIT_FDC_MASK);
        // keep the flag if fdc_mask becomes on.
        fdc_mask_core1 &= (g_pio_parallel_write.idata & FDC_UNIT_FDC_MASK);
        break;
    case FDC_8INCH_HALT:
        // halt on forcely 
...
{
    g_pio_parallel_read.odata[FDC_MB8877_DR] = g_pio_parallel_read.odata[FDC_8INCH_NDATA];
}
...
static void fdc_cmd_readdata_headengd()
{
    ...
    fdc_cancel_lost_event();
//  printf("FDC8: Read data: Pre_Sts:%02x After:%u\n", dev->pre_sts, usec);
}
...
static void __not_in_flash_func(fdc_send_data_reg)()
{
    ...
    dat = ~dat;
#endif
    // data is stored in the next data register 
    // set to DR when fire DRQ
    msg_send_data_to_core1(MSG_TYPE_PARALLEL_WRITE, FDC_8INCH_NDATA, dat);
}
...
static void __not_in_flash_func(fdc_send_data_reg_sync)()
{
    ...
    // data is stored in the next data register 
    // set to DR when fire DRQ
    msg_send_data_to_core1(MSG_TYPE_PARALLEL_WRITE_SYNC, FDC_8INCH_NDATA, dat);
}
...
static void __not_in_flash_func(fdc_set_irq)(uint8_t val)
{
    if (dev->now_irq != val) {
        dev->now_irq = val;
//      msg_send_data_to_core1(MSG_TYPE_PARALLEL_WRITE, FDC_8INCH_IRQ, ~dev->now_irq);
        if (val) {
//            if (is_halt_signal_on()) {
//                uint32_t tim = to_us_since_boot(get_absolute_time()) & 0xffffffff;
//                printf("HALT OFF IRQ: TIM:%u C:%d H:%d R:%d POS:%d STS:%02x DST:%02x\n"
//                , tim
//                , dev->trkreg, dev->sid_num, dev->secreg, dev->data_idx, dev->status, dev->drv_sts);
//            }
            fdc_common_set_irq();
...
//            if (is_halt_signal_on()) {
//                uint32_t tim = to_us_since_boot(get_absolute_time()) & 0xffffffff;
//                printf("HALT OFF DRQ: TIM:%u C:%d H:%d R:%d POS:%d STS:%02x DST:%02x\n"
//                , tim
//                , dev->trkreg, dev->sid_num, dev->secreg, dev->data_idx, dev->status, dev->drv_sts);
//            }
            halt_signal_off(HALT_SIGNAL_8INCH);
...
void fdc_8inch_get_info()
{
    ...
    printf("CMDREG:%02x TRKREG:%02x SECREG:%02x STATUS:%02x\n",
        ...
        dev->secreg,
        dev->status);
    printf("UNIT: DRV:%d SIDE:%d DENSITY:%d FDCMASK:%02x NMIEN:%02x\n",
        dev->drv_num,
        dev->sid_num,
        dev->density,
        dev->fdc_mask,
