...
{
    uint32_t nsts = 0;
    if (sts & FDC_5INCH_ST_BUSY) nsts |= FDCC_ST_BUSY;
    if (sts & FDC_5INCH_ST_SEEKERR) nsts |= FDCC_ST_SEEKERR;
    if (sts & FDC_5INCH_ST_CRCERR) nsts |= FDCC_ST_CRCERR;
    if (sts & FDC_5INCH_ST_WRITEP) nsts |= FDCC_ST_WRITEP;
    if (sts & FDC_5INCH_ST_NOTREADY) nsts |= FDCC_ST_NOTREADY;
    return nsts;
}
...
static uint32_t convert_status_read_sector(uint8_t sts)
{
    uint32_t nsts = 0;
    if (sts & FDC_5INCH_ST_BUSY) nsts |= FDCC_ST_BUSY;
    if (sts & FDC_5INCH_ST_LOSTDATA) nsts |= FDCC_ST_LOSTDATA;
    if (sts & FDC_5INCH_ST_CRCERR) nsts |= FDCC_ST_CRCERR;
    if (sts & FDC_5INCH_ST_RECNFND) nsts |= FDCC_ST_RECNFND;
    if (sts & FDC_5INCH_ST_RECTYPE) nsts |= FDCC_ST_DELETED;
    if (sts & FDC_5INCH_ST_NOTREADY) nsts |= FDCC_ST_NOTREADY;
    return nsts;
}
...
static uint32_t convert_status_write_sector(uint8_t sts)
{
    ...
    if (sts & FDC_5INCH_ST_CRCERR) nsts |= FDCC_ST_CRCERR;
    if (sts & FDC_5INCH_ST_RECNFND) nsts |= FDCC_ST_RECNFND;
    if (sts & FDC_5INCH_ST_WRITEFAULT) nsts |= FDCC_ST_WRITEP;
    if (sts & FDC_5INCH_ST_WRITEP) nsts |= FDCC_ST_WRITEP;
    if (sts & FDC_5INCH_ST_NOTREADY) nsts |= FDCC_ST_NOTREADY;
    return nsts;
}
...
{
    parallel_write(FDC_5INCH_UNIT, drv & ~FDC_5INCH_UNIT_MOTOR);
}

bool fdc5inch_motor_on(uint8_t drv, uint32_t sid_num, bool dden)
{
    uint8_t sts;
    uint8_t data = (drv & 3) | FDC_5INCH_UNIT_MOTOR;
    if (dden) data |= FDC_5INCH_UNIT_DDEN;
    if (sid_num) data |= FDC_5INCH_UNIT_SIDE1;
    if (!some_signals_fdc_is_enable()) {
        printf("CMD_FDC5: Device is not active.\n");
        return false;
    }
    if (g_shell_cmd_fdc.verbose) printf("CMD_FDC5: Motor On: 0x%02x\n", data);
    parallel_write(FDC_5INCH_UNIT, data);
    for(int i=0; i< (5 * 1000 * 1000 / FDC_WAIT); i++) {
        wait_spin_us(FDC_WAIT);
        sts = FDC_VALUE(parallel_read(FDC_5INCH_STR));
        if (!(sts & FDC_5INCH_ST_NOTREADY)) break;
    }
    if (sts & FDC_5INCH_ST_NOTREADY) {
        printf("CMD_FDC5: Drive not ready: Sts:0x%02x\n", sts);
        fdc5inch_motor_off(data);
        return false;
    }
    return true;
}
...
static uint8_t fdc_wait_busy()
{
    ...
    for(int i=0; i< (5 * 1000 * 1000 / FDC_WAIT); i++) {
        wait_spin_us(FDC_WAIT);
        sts = FDC_VALUE(parallel_read(FDC_5INCH_STR));
        if (sts & FDC_5INCH_ST_BUSY) break;
    }
    if (!(sts & FDC_5INCH_ST_BUSY)) {
        printf("CMD_FDC5: Cannot start command: Sts:0x%02x\n", sts);
    }
    return sts;
}
...
static uint8_t fdc_wait_idle()
{
    ...
    if (sts & FDC_5INCH_ST_BUSY) {
        printf("CMD_FDC5: Cannot stop command: Sts:0x%02x\n", sts);
    }
    return sts;
}
...
void fdc5inch_forceint()
{
    // forceint
    uint8_t data = 0xd0;
    printf("CMD_FDC5: Forceint: 0x%02x\n", data);
    parallel_write(FDC_5INCH_CR, (uint8_t)FDC_VALUE(data));
    sleep_us(500);
    printf("CMD_FDC5: Done.\n");
}
...
static uint8_t fdc_wait_seek_end()
{
    ...
    for(int i=0; i< (20 * 1000 * 1000 / FDC_WAIT); i++) {
        ...
        sts = FDC_VALUE(parallel_read(FDC_5INCH_STR));
        if (!(sts & FDC_5INCH_ST_BUSY)) break;
        if (sts & (FDC_5INCH_ST_SEEKERR | FDC_5INCH_ST_CRCERR)) break;
    }
    return sts;
}
...
{
    FDC_VALUE(parallel_read(FDC_5INCH_STR));
}
...
bool fdc5inch_restore(uint8_t drv, uint32_t step_rate, pftime_t *ptime)
{
    ...
    if (g_shell_cmd_fdc.verbose) printf("CMD_FDC5: Restore: 0x%02x\n", data);
    if (ptime) {
        ptime->start = to_us_since_boot(get_absolute_time());
        ptime->count = 0;
    }
    parallel_write(FDC_5INCH_TR, (uint8_t)FDC_VALUE(g_shell_cmd_fdc.trk[drv]));
    wait_spin_us(2);
    parallel_write(FDC_5INCH_CR, (uint8_t)FDC_VALUE(data));
    sts = fdc_wait_busy();
    if (!(sts & FDC_5INCH_ST_BUSY)) {
    ...
    if (sts & (FDC_5INCH_ST_BUSY | FDC_5INCH_ST_SEEKERR | FDC_5INCH_ST_CRCERR)) {
        goto eof;
    }
    rc = true;
eof:
...
bool fdc5inch_seek(uint8_t drv, uint32_t trk_num, uint32_t step_rate, pftime_t *ptime)
{
    ...
    if (g_shell_cmd_fdc.verbose) printf("CMD_FDC5: Seek: 0x%02x\n", data);
    if (ptime) {
        ptime->start = to_us_since_boot(get_absolute_time());
        ptime->count = 0;
    }
    parallel_write(FDC_5INCH_TR, (uint8_t)FDC_VALUE(g_shell_cmd_fdc.trk[drv]));
    wait_spin_us(2);
    parallel_write(FDC_5INCH_DR, (uint8_t)FDC_VALUE(trk_num));
    wait_spin_us(2);
    parallel_write(FDC_5INCH_CR, (uint8_t)FDC_VALUE(data));
    sts = fdc_wait_busy();
    if (!(sts & FDC_5INCH_ST_BUSY)) {
...
bool fdc5inch_step(uint8_t drv, int dir, uint32_t step_rate, pftime_t *ptime)
{
    ...
    data |= (step_rate & 0x03);
    if (g_shell_cmd_fdc.verbose) printf("CMD_FDC5: Step: 0x%02x\n", data);
    if (ptime) {
        ptime->start = to_us_since_boot(get_absolute_time());
        ptime->count = 0;
    }
...
bool fdc5inch_read_data(uint8_t drv, uint32_t sid_num, uint32_t sec_num, pftime_t *ptime)
{
    ...
    if (g_shell_cmd_fdc.verbose) printf("CMD_FDC5: Read Data: 0x%02x\n", data);
    if (ptime) {
        ptime->start = to_us_since_boot(get_absolute_time());
        ptime->count = 0;
    }
    parallel_write(FDC_5INCH_TR, (uint8_t)FDC_VALUE(trk_num));
    wait_spin_us(2);
    parallel_write(FDC_5INCH_SCR, (uint8_t)FDC_VALUE(sec_num));
    wait_spin_us(2);
    parallel_write(FDC_5INCH_CR, (uint8_t)FDC_VALUE(data));
    sts = fdc_wait_busy();
    if (!(sts & FDC_5INCH_ST_BUSY)) {
    ...
    wait_spin_us(2);
    sts = FDC_VALUE(parallel_read(FDC_5INCH_STR));
    if (!(sts & (FDC_5INCH_ST_RECNFND | FDC_5INCH_ST_CRCERR))) {
//      printf("CMD_FDC5: Reading: sts:0x%02x\n", sts);
        while(g_shell_cmd_fdc.buffer.count < 2048) {
            irqdrq = fdc_wait_drqirq();
            if (irqdrq & FDC_5INCH_UNIT_IRQ) {
                wait_spin_us(2);
                sts = FDC_VALUE(parallel_read(FDC_5INCH_STR));
                rc = true;
                break;
            } else if (irqdrq & FDC_5INCH_UNIT_DRQ) {
                // read data
                wait_spin_us(2);
                data = FDC_VALUE(parallel_read(FDC_5INCH_DR));
                if (g_shell_cmd_fdc.buffer.count < 256) {
        ...
        printf("CMD_FDC5: irq_drq:0x%02x\n", irqdrq);
    }
    sts = fdc_wait_idle();
eof:
    if (ptime) {
...
bool fdc5inch_write_data(uint8_t drv, uint32_t sid_num, uint32_t sec_num, pftime_t *ptime)
{
    ...
    if (g_shell_cmd_fdc.verbose) printf("CMD_FDC5: Write Data: 0x%02x\n", data);
    if (ptime) {
        ptime->start = to_us_since_boot(get_absolute_time());
        ptime->count = 0;
    }
    ...
    wait_spin_us(2);
    sts = FDC_VALUE(parallel_read(FDC_5INCH_STR));
    if (!(sts & (FDC_5INCH_ST_WRITEP | FDC_5INCH_ST_RECNFND | FDC_5INCH_ST_CRCERR))) {
//      printf("CMD_FDC5: Writing: sts:0x%02x\n", sts);
        while(g_shell_cmd_fdc.buffer.count < 2048) {
            irqdrq = fdc_wait_drqirq();
//          printf("CMD_FDC5: irq_drq:0x%02x\n", irqdrq);
            if (irqdrq & FDC_5INCH_UNIT_IRQ) {
                ...
                wait_spin_us(2);
                parallel_write(FDC_5INCH_DR, FDC_VALUE(data));
                if (ptime) {
                    ptime->data[ptime->count] = (uint32_t)(to_us_since_boot(get_absolute_time()) - ptime->start);
                    ptime->count++;
                }
...
bool fdc5inch_read_track(uint8_t drv, uint32_t sid_num, pftime_t *ptime)
{
    ...
    // read data
    uint8_t data = 0xe0;
    if (g_shell_cmd_fdc.verbose) printf("CMD_FDC5: Read Track: 0x%02x\n", data);
    if (ptime) {
        ptime->start = to_us_since_boot(get_absolute_time());
        ptime->count = 0;
    }
...
bool fdc5inch_read_addr(uint8_t drv, pftime_t *ptime)
{
    ...
    // read data
    uint8_t data = 0xc0;
    if (g_shell_cmd_fdc.verbose) printf("CMD_FDC5: Read Address: 0x%02x\n", data);
    if (ptime) {
        ptime->start = to_us_since_boot(get_absolute_time());
        ptime->count = 0;
    }
...
static uint8_t fdc_wait_drqirq()
{
    ...
    do {
        wait_spin_us(3);
        tight_loop_contents();
        irqdrq = (~parallel_read(FDC_5INCH_UNIT)) & 0xff;
    } while((irqdrq & (FDC_5INCH_UNIT_IRQ | FDC_5INCH_UNIT_DRQ)) == 0);
    return irqdrq;
}
...
static uint8_t fdc_wait_irq()
{
    ...
    do {
        ...
        if (fdc_common_now_irq()) {
            irqdrq |= FDC_5INCH_UNIT_IRQ;
        }
    } while((irqdrq & FDC_5INCH_UNIT_IRQ) == 0);
    return irqdrq;
}
...
void fdc5inch_read_status()
{
    ...
    printf("CMD_FDC5: Status: 0x%02x\n", sts);
}

void fdc5inch_unitsel(uint8_t opts)
{
    printf("CMD_FDC5: Write to unit sel 0x%02x\n", opts);
    parallel_write(FDC_5INCH_UNIT, opts);
}

//======================================================================

#define SIDES_PER_DISK 2
#define SECTORS_PER_GROUP 8
...
bool fdc5inch_files_entry_cb(int row, void *p_entry, void *user_data)
{
    ...
    uint32_t size = inf->sector_size * secs;
    if (entry->end_bytes) {
        uint16_t eb = BE_TO_LE_16(entry->end_bytes);
        size = size + eb - inf->sector_size;
    }
    printf("%s %d%c %2d (%6uB)  "
        ,file_name
        ,entry->type & 7
...
void fdc5inch_cmd_load(uint8_t drv, const char *file_name)
{
    ...
    // motor on
    if (!fdc5inch_motor_on(drv, 0, true)) {
        return;
    }
    // read FAT
    if (!read_fat_table(drv)) {
    ...
    while(grp < 0xc0 && !last) {
        ...
        // seek
        rc = fdc5inch_seek(drv, trk, 1, NULL);
        if (!rc) {
            break;
        }
        // set side1 signal
        fdc5inch_motor_on(drv, sid, true);
        // read data
        for(int sec = start_sec; sec < start_sec + secs && !last; sec++) {
            rc = fdc5inch_read_data(drv, sid, sec, NULL);
            if (!rc) {
                last = true;
                break;
            }
...
bool fdc5inch_cmd_boottest(int boot_type)
{
    ...
    if (boot_type != 1) {
        printf("CMD_FDC5: Boot type %d is not supported.\n", boot_type);
        return false;
    }
    ...
    bool rc = true;
    do {
        parallel_write(FDC_5INCH_SCR, 0);
        wait_spin_us(6);
        parallel_write(FDC_5INCH_UNIT, 0xa8);
        wait_spin_us(16);
        fdctyp = parallel_read(FDC_5INCH_SCR);
        wait_spin_us(6);
        parallel_write(FDC_5INCH_UNIT, 0x28);
        wait_spin_us(4);
        parallel_write(FDC_5INCH_CR, 0x2f); // $D0 Force int
        printf("FDC type is 0x%02x.\n", fdctyp);
        wait_spin_us(1000000);  // wait
        sts = parallel_read(FDC_5INCH_STR);
        if ((sts & 0x80) == 0) {
            // not ready ?
        ...
        wait_spin_us(22);
        parallel_write(FDC_5INCH_CR, 0xfe); // $01 Restore
        ...
        do {
            pos = 0;
            wait_spin_us(18);
            parallel_write(FDC_5INCH_SCR, ~(sec_num + 1)); // sector
            wait_spin_us(8);
            parallel_write(FDC_5INCH_CR, 0x7f); // $80 Read sector
            do {
                sts = fdc_wait_drqirq();
                if (sts & FDC_5INCH_UNIT_DRQ) {
                    wait_spin_us(3);
                    data = parallel_read(FDC_5INCH_DR);
                    buffer[pos] = ~data;
                    pos++;
                } 
            } while((sts & FDC_5INCH_UNIT_IRQ) == 0);
            wait_spin_us(4);
            parallel_write(FDC_5INCH_UNIT, 0x28);
            wait_spin_us(16);
            sts = parallel_read(FDC_5INCH_STR);
        ...
        for(trk_num=1; trk_num<2; trk_num++) {
            wait_spin_us(3);
            parallel_write(FDC_5INCH_DR, ~trk_num);
            wait_spin_us(3);
            parallel_write(FDC_5INCH_CR, 0xee);    // $12 Seek
            fdc_wait_irq();
            wait_spin_us(16);
            sts = parallel_read(FDC_5INCH_STR);
            sts = ~sts;
            ...
            for(sec_num=0; sec_num<32; sec_num++) {
                wait_spin_us(3);
                parallel_write(FDC_5INCH_TR, ~trk_num);
                pos = 0;
                wait_spin_us(10);
                parallel_write(FDC_5INCH_UNIT, 0x28 | (sec_num >= 16 ? 0x10: 0));
                wait_spin_us(50000);
                parallel_write(FDC_5INCH_SCR, ~((sec_num % 16) + 1));
                wait_spin_us(6);
                parallel_write(FDC_5INCH_CR, 0x7f); // $80 Read sector
                do {
                    sts = fdc_wait_drqirq();
                ...
                if (xaddr >= 0x3500) {
                    wait_spin_us(3);
                    parallel_write(FDC_5INCH_UNIT, 0);   // Motor off
                    last = true;
                    break;
                }
            }
